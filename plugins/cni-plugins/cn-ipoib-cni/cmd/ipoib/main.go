...
import (
	...
	"github.com/cornelis/cn-ipoib-cni/pkg/config"
	"github.com/cornelis/cn-ipoib-cni/pkg/ipoib"
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	ibLink, err := ipoibManager.CreateIpoibLink(n, args.IfName, netns, args.ContainerID)
	if err != nil {
		return err
	}
...
func cmdDel(args *skel.CmdArgs) error {
	// Log that cmdDel was called
	fmt.Fprintf(os.Stderr, "[CNI-IPOIB-DEL] === cmdDel CALLED === ifName=%s containerID=%s netns=%s pid=%d\n",
		args.IfName, args.ContainerID, args.Netns, os.Getpid())

	n, _, err := config.LoadConf(args.StdinData)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[CNI-IPOIB-DEL] LoadConf failed: %v\n", err)
		return err
	}
	...
	if isIpamProvided {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "[CNI-IPOIB-DEL] IPAM ExecDel failed: %v\n", err)
			return err
		}
	}
	...
	if err != nil {
		...
		if ok {
			fmt.Fprintf(os.Stderr, "[CNI-IPOIB-DEL] Netns not found, calling CleanupHostNamespace\n")
			return ipoibManager.CleanupHostNamespace(args.IfName, args.ContainerID)
		}

		fmt.Fprintf(os.Stderr, "[CNI-IPOIB-DEL] Failed to open netns: %v\n", err)
		return fmt.Errorf("failed to open netns %s: %q", netns, err)
	}
	defer netns.Close()

	fmt.Fprintf(os.Stderr, "[CNI-IPOIB-DEL] Calling RemoveIpoibLink\n")
	err = ipoibManager.RemoveIpoibLink(args.IfName, netns)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[CNI-IPOIB-DEL] RemoveIpoibLink failed: %v, calling CleanupHostNamespace\n", err)
		_ = ipoibManager.CleanupHostNamespace(args.IfName, args.ContainerID)
	} else {
		fmt.Fprintf(os.Stderr, "[CNI-IPOIB-DEL] RemoveIpoibLink succeeded\n")
	}
	return err
}
