...
import (
	types "github.com/cornelisnetworks/cn-fabric-k8s/plugins/device-plugins/cn-rdma-shared-dev-plugin/pkg/types"
...
// CreateCDISpec provides a mock function with given fields: resourcePrefix, resourceName, poolName, devices, envVars
func (_m *CDI) CreateCDISpec(resourcePrefix string, resourceName string, poolName string, devices []types.PciNetDevice, envVars []string) error {
	ret := _m.Called(resourcePrefix, resourceName, poolName, devices, envVars)

	var r0 error
	if rf, ok := ret.Get(0).(func(string, string, string, []types.PciNetDevice, []string) error); ok {
		r0 = rf(resourcePrefix, resourceName, poolName, devices, envVars)
	} else {
		r0 = ret.Error(0)
	}
