...
import (
	...
	"github.com/cornelisnetworks/cn-fabric-k8s/plugins/device-plugins/cn-rdma-shared-dev-plugin/pkg/cdi"
...
var _ = Describe("Cdi", func() {
	Context("successfully create CDI spec file", func() {
		It("should create CDI spec file", func() {
			...
			device.On("GetHfiSpec").Return([]*pluginapi.DeviceSpec{})

			err := cdi.New().CreateCDISpec("test-prefix", "test-name", "test-pool", []types.PciNetDevice{&device}, []string{})
...
kind: test-prefix/test-name`
			...
			// /sys must be exposed read-only via rbind so OPX can resolve
			// /sys/class/infiniband/hfi1_* symlinks into /sys/devices/...
			Expect(string(cdiSpec)).To(ContainSubstring("containerPath: /sys"))
			Expect(string(cdiSpec)).To(ContainSubstring("hostPath: /sys"))
			Expect(string(cdiSpec)).To(ContainSubstring("- rbind"))
			Expect(string(cdiSpec)).To(ContainSubstring("- ro"))
		})
		It("should inject FI_PROVIDER globally but never FI_OPX_HFI_SELECT", func() {
			staticDir := "cdi_static"
			dynamicDir := "cdi_dynamic"
			fs := &utils.FakeFilesystem{
				Dirs: []string{staticDir, dynamicDir},
			...
			envVars := []string{"FI_PROVIDER=opx", "FI_OPX_HFI_SELECT=0"}
			err := cdi.New().CreateCDISpec("test-prefix", "test-name", "test-pool", []types.PciNetDevice{&device}, envVars)
			Expect(err).NotTo(HaveOccurred())

			cdiSpec, err := os.ReadFile(fs.RootDir + "/" + dynamicDir + "/test-prefix_test-pool.yaml")
			Expect(err).NotTo(HaveOccurred())

			Expect(string(cdiSpec)).To(ContainSubstring("FI_PROVIDER=opx"))
			Expect(string(cdiSpec)).NotTo(ContainSubstring("FI_OPX_HFI_SELECT"))
		})
	})
