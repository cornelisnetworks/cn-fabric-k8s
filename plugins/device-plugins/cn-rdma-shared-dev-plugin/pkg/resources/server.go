...
func (rs *resourceServer) updateCDISpec() error {
	...
	err := rs.cdi.CreateCDISpec(cdiResourcePrefix, cdiResourceKind, rs.cdiResourceName, rs.pciDevices, rs.envVars)
	if err != nil {
	...
	*pluginapi.AllocateResponse, error) {
	...
	for i := range r.GetContainerRequests() {
		...
		if rs.useCdi {
			...
			// OPX follows /sys/class/infiniband/hfi1_* symlinks into
			// /sys/devices/...; mounting only /sys/class/infiniband leaves
			// those targets dangling, so the full /sys is required.
			ress[i].Mounts = append(ress[i].Mounts, &pluginapi.Mount{
				ContainerPath: "/sys",
				HostPath:      "/sys",
				ReadOnly:      true,
			})

			envVars := cdi.FilterGlobalEnv(rs.envVars)
			if len(envVars) > 0 {
...
func getDevicesSpec(devices []types.PciNetDevice) []*pluginapi.DeviceSpec {
	...
	for _, device := range devices {
		...
		// Add HFI device specs (/dev/hfi1_*)
