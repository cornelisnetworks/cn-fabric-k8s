...
import (
	...
	cdiMocks "github.com/cornelisnetworks/cn-fabric-k8s/plugins/device-plugins/cn-rdma-shared-dev-plugin/pkg/cdi/mocks"
	"github.com/cornelisnetworks/cn-fabric-k8s/plugins/device-plugins/cn-rdma-shared-dev-plugin/pkg/types"
	"github.com/cornelisnetworks/cn-fabric-k8s/plugins/device-plugins/cn-rdma-shared-dev-plugin/pkg/types/mocks"
...
var _ = Describe("resourceServer tests", func() {
	...
	fakePciDevice.On("GetHfiSpec").Return([]*pluginapi.DeviceSpec{})
	...
	Context("ListAndWatch", func() {
		...
		It("Ensure that CDI spec is updated on ListAndWatch call", func() {
			...
			cdi := &cdiMocks.CDI{}
			cdi.On("CreateCDISpec", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
			rs := obj.(*resourceServer)
			rs.cdi = cdi
			...
			err = rs.ListAndWatch(nil, s)
			cdi.AssertCalled(GinkgoT(), "CreateCDISpec", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything)
			Expect(err).ToNot(HaveOccurred())
		})
	...
	Context("Allocate", func() {
		...
		It("non-CDI Allocate exposes /sys read-only and propagates env vars", func() {
			rs := resourceServer{
				resourceName: "fake",
				socketName:   "fake.sock",
				envVars:      []string{"FI_PROVIDER=opx", "FI_OPX_HFI_SELECT=0", "MALFORMED", "=NOKEY", "FOO=bar=baz"},
			}
			req := &pluginapi.AllocateRequest{
				ContainerRequests: []*pluginapi.ContainerAllocateRequest{nil},
			}
			res, err := rs.Allocate(context.TODO(), req)
			Expect(err).ToNot(HaveOccurred())
			Expect(res.ContainerResponses).To(HaveLen(1))
			cr := res.ContainerResponses[0]

			Expect(cr.Mounts).To(ContainElement(&pluginapi.Mount{
				ContainerPath: "/sys",
				HostPath:      "/sys",
				ReadOnly:      true,
			}))

			Expect(cr.Envs).To(HaveKeyWithValue("FI_PROVIDER", "opx"))
			Expect(cr.Envs).To(HaveKeyWithValue("FOO", "bar=baz"))
			Expect(cr.Envs).NotTo(HaveKey("FI_OPX_HFI_SELECT"))
			Expect(cr.Envs).NotTo(HaveKey("MALFORMED"))
			Expect(cr.Envs).NotTo(HaveKey(""))
		})
		It("non-CDI Allocate omits Envs when no env vars are configured", func() {
			rs := resourceServer{resourceName: "fake", socketName: "fake.sock"}
			req := &pluginapi.AllocateRequest{
				ContainerRequests: []*pluginapi.ContainerAllocateRequest{nil},
			}
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res.ContainerResponses).To(HaveLen(1))
			Expect(res.ContainerResponses[0].Envs).To(BeEmpty())
			Expect(res.ContainerResponses[0].Mounts).To(HaveLen(1))
		})
	})
